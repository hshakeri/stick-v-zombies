...
test('the main PC uses Alan-era blue desktop cues', () => {
  const manager = new StageManager();
  manager.loadStage(1);
  const fills = [];
  const { context, calls } = createCanvasContextMock();
  context.fillRect = (...args) => fills.push({ color: context.fillStyle, args });
  ...
  assert.equal(calls.save, calls.restore, 'desktop paint must not leak canvas state');
});
