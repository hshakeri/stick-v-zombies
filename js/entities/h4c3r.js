...
/**
 * Final campaign boss. H4C3R is intentionally deterministic and readable:
 * every damaging move has a long cyan preview, can hit only once, and is
 * followed by a generous punish window.
 */
export class H4C3R {
  constructor(x, y) {
    this.x = x;
  ...
  update(dt, groundY, player, sketchBlocks, camera, platforms = [], enemies = [], friendlyTargets = []) {
    ...
    // Green's stun creates a real opening, but bosses cap its duration so one
    // summon cannot skip an entire phase.
  ...
  updateAI(dt, player, camera) {
    ...
    switch (this.state) {
      case 'idle': {
        ...
        // A small stalking step keeps the arena alive without smothering the
        // player during the actual recovery window.
  ...
  startPacketDash(player, camera) {
    ...
    // If the player is already at an arena edge, cross past them from the
    // other direction instead of producing a tiny, unreadable dash.
  ...
  takeDamage(amount, knockbackDir = 1, knockbackPower = 200, isCrit = false) {
    ...
    // H4C3R can be moved during neutral/recovery, but a hit never cancels an
    // attack's warning and turns it into a surprise.
  ...
  clearOwnedProjectiles() {
    // Future H4C3R projectile variants can opt into this cleanup by setting
    // owner. Keeping the filter here also prevents a defeated boss from
    // leaving a late hit behind during the victory transition.
  ...
  drawDashDecoys(ctx) {
    ...
    // Exactly two harmless silhouettes communicate extreme speed without
    // adding entities, collision work, or extra threats.
