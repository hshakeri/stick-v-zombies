export class StickFigureRenderer {
  constructor(color = '#ff7700', strokeWidth = 5, scale = 1.0, isHollowHead = false) {
	...
	this.scale = scale;
	this.glowColor = null;
	this.isHunched = false;
	this.isZombie = false;
	this.isHollowHead = isHollowHead;
  ...
  computePoseBones(pose, timer, isGrounded, isHurt) {
	...
	switch (pose) {
	  ...
	  case 'attack_jab': {
		...
		setPoint(bones.handL, 34, -42);
		setPoint(bones.elbowR, -8, -34);
		setPoint(bones.handR, 4, -38);
		setPoint(bones.footL, 14, 0);
	  ...
	  case 'attack_cross': {
		...
		setPoint(bones.handL, 8, -42);
		setPoint(bones.elbowR, 22, -40);
		setPoint(bones.handR, 44, -38);
