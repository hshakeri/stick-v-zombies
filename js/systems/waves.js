...
export class WaveDirector {
  ...
  generateWaveQueue(wave) {
    if (wave === 10) {
      ...
      for (let i = 0; i < 8; i++) {
        this.spawnQueue.push({ type: i % 2 === 0 ? 'runner' : 'spitter', delay: 2.5 + i * 2.0 });
      }
      this.spawnQueue.push({ type: 'brute', delay: 8.0 });
      return;
    }
    ...
    if (isBossWave) {
      ...
      for (let i = 0; i < 4 + wave; i++) {
        this.spawnQueue.push({ type: i % 2 === 0 ? 'runner' : 'spitter', delay: 2.0 + i * 1.5 });
      }
      return;
    }
  ...
  spawnZombie(type, player, groundY, camera = null) {
    // Spawn distributed from left or right with varied spacing
    const side = Math.random() > 0.5 ? 1 : -1;
    let spawnX = player ? player.x + side * (400 + Math.random() * 300) : side * (400 + Math.random() * 300);
    spawnX = Math.max(-920, Math.min(920, spawnX));
